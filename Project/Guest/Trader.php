...
if(isset($_POST["txt_submit"]))
{
	...
	move_uploaded_file($path,'../Assets/Files/AgencyDocs/'.$photo);
  
	$proof=$_FILES['txt_proof']['name'];
	$path=$_FILES['txt_proof']['tmp_name'];
	move_uploaded_file($path,'../Assets/Files/AgencyDocs/'.$proof);
  ...
  values('".$name."', '".$email."', '".$contact."', '".$addrs."',  '".$photo."', '".$proof."', '".$password."', '".$place."')";
 ...
 <script type="text/javascript">
    ...
    function checkPasswordStrength() {
        ...
        const pattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$/;
            ...
            passwordFeedback.textContent = "Password must be at least 6 characters and include uppercase, lowercase, a number, and a special character.";
...
<body>
  <section class="container">
    <header class="header">Register Now as a Trader</header>
    ...
    <div class="file-input">
        <input type="file" id="proof" name="txt_proof" required accept="image/jpeg, image/png, image/gif" onchange="validateProof(); showProofPreview()" />
