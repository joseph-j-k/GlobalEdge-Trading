...
<head>
    ...
    <style>
        ...
        body {
            ...
            background-color: #f0f2f5; /* Optional: background color for the page */
        }
...
<script>
    ...
    const adminEmails = <?php echo json_encode($adminEmails); ?>;
    const adminPasswords = <?php echo json_encode($adminPasswords); ?>;
    const userEmails = <?php echo json_encode($userEmails); ?>;
    const userPasswords = <?php echo json_encode($userPasswords); ?>;
    const traderEmails = <?php echo json_encode($traderEmails); ?>;
    const traderPasswords = <?php echo json_encode($traderPasswords); ?>;
    ...
    function checkEmailAndPassword() {
        ...
        // Check if the email exists in any of the arrays
        if (adminEmails.includes(enteredEmail)) {
            emailValid = true;
            const index = adminEmails.indexOf(enteredEmail);
            passwordValid = enteredPassword === adminPasswords[index];
        } else if (userEmails.includes(enteredEmail)) {
            emailValid = true;
            const index = userEmails.indexOf(enteredEmail);
            passwordValid = enteredPassword === userPasswords[index];
        } else if (traderEmails.includes(enteredEmail)) {
            emailValid = true;
            const index = traderEmails.indexOf(enteredEmail);
            passwordValid = enteredPassword === traderPasswords[index];
        }

        // Set feedback messages
        emailFeedback.textContent = emailValid ? "Email is correct." : "Incorrect email.";
        emailFeedback.className = "feedback " + (emailValid ? "valid" : "invalid");
        passwordFeedback.textContent = passwordValid ? "Correct password." : "Incorrect password.";
        passwordFeedback.className = "feedback " + (passwordValid ? "valid" : "invalid");
    }
